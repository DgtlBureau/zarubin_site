...
const getAllPosts = () => {
  const postMetadata = getComplianceMetadata();
  return postsSorting(postMetadata);
};

export async function generateStaticParams(): Promise<Slug[]> {
  const compliances = getComplianceMetadata();
  return compliances.map((post) => ({ slug: post.slug }));
}
...
const getPostContent = (slug: string) => {
  const folder = `src/${MenuItems.PLAYBOOK.toLowerCase()}/compliance/`;
...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${BASE_URL}/${MenuItems.PLAYBOOK.toLowerCase()}/compliance/${slug}`,
    },
    openGraph: {
      type: 'article',
      locale: 'en_US',
      ...
      title,
      description,
      url: `${BASE_URL}/${MenuItems.PLAYBOOK.toLowerCase()}/compliance/${slug}`,
...
export default function CompliancePostPage(props: { params: { slug: string } }) {
  ...
  const articleUrl = `${BASE_URL}/${MenuItems.PLAYBOOK.toLowerCase()}/compliance/${slug}`;
